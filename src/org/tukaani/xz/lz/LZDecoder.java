...
public final class LZDecoder {
    private byte[] buf;
    private int bufSize; // To avoid buf.length with an array-cached buf.
    private int start = 0;
    private int pos = 0;
    private int full = 0;
    private int limit = 0;
    ...
    public boolean equals(Object o) {
        ...
        LZDecoder lzDecoder = (LZDecoder) o;
        return bufSize == lzDecoder.bufSize && start == lzDecoder.start && pos == lzDecoder.pos && full == lzDecoder.full && limit == lzDecoder.limit && pendingLen == lzDecoder.pendingLen && pendingDist == lzDecoder.pendingDist && Arrays.equals(buf, lzDecoder.buf);
    }

    @Override
    public int hashCode() {
        int result = Objects.hash(bufSize, start, pos, full, limit, pendingLen, pendingDist);
