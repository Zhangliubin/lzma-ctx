...
abstract class LZMACoder {
    ...
    void reset() {
        ...
        for (short[] match : isMatch) {
            RangeCoder.initProbs(match);
        }
        ...
        for (short[] shorts : isRep0Long) {
            RangeCoder.initProbs(shorts);
        }

        for (short[] distSlot : distSlots) {
            RangeCoder.initProbs(distSlot);
        }

        for (short[] shorts : distSpecial) {
            RangeCoder.initProbs(shorts);
        }
